...
Write-Host "`n🚀 Unregistering $selected..." -ForegroundColor Green

try {
    wsl --unregister $selected
}
catch {
    Write-Host "`n❌ Failed to unregister distribution!" -ForegroundColor Red
    Write-Host "Error: $_"
    Write-Host "`nTroubleshooting steps:`n1. Verify WSL is enabled`n2. Check distribution exists: wsl --list`n3. Try launching manually: wsl --unregister '$selected'"
}
